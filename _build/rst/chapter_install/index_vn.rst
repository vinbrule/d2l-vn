...
`Miniconda <https://conda.io/en/latest/miniconda.html>`__. Phiên bản
...
``sh <FILENAME> -b``. Với người dùng macOS:
...
nguồn từ `đường dẫn này <https://d2l.ai/d2l-en-0.7.0.zip>`__ và giải
nén. Một cách khác, nếu bạn đã có cài đặt sẵn ``unzip`` (nếu chưa, hãy
chạy lệnh ``sudo apt install unzip``):
...
Bây giờ, ta sẽ kích hoạt môi trường ``d2l`` và cài đặt ``pip``. Hãy nhập
``y`` cho phần truy vấn theo sau lệnh này:
...
trên một máy chủ GPU, hãy tiến hành theo :ref:`sec_gpu` để cài đặt
...
Ta cũng sẽ cài đặt gói thư viện ``d2l`` mà bao gồm các hàm và lớp thường
...
Bây giờ, bạn có thể truy cập vào địa chỉ http://localhost:8888 (thường
...
lệnh ``conda activate d2l`` để kích hoạt môi trường trước khi chạy mã
nguồn trong sách cũng như khi cập nhật MXNet hoặc gói thư viện ``d2l``.
...
1. Đường dẫn https://d2l.ai/d2l-en.zip luôn luôn trỏ đến phiên bản mới
   nhất.
2. Để cập nhật gói thư viện ``d2l`` hãy sử dụng lệnh
   ``pip install d2l --upgrade``.
3. Đối với phiên bản CPU, MXNet có thể được cập nhật bằng lệnh
...
`CUDA <https://developer.nvidia.com/cuda-downloads>`__, thì bạn nên cài
...
thông qua lệnh ``nvcc --version`` hoặc
``cat /usr/local/cuda/version.txt``. Giả sử, bạn đã cài đặt CUDA 10.1,
...
``pip install -U --pre mxnet-cu101``. Bạn có thể thay đổi các chữ số
cuối theo phiên bản CUDA của bạn, ví dụ, ``cu100`` cho CUDA phiên bản
10.0 và ``cu90`` cho CUDA phiên bản 9.0. Bạn có thể tìm thấy tất cả các
phiên bản MXNet có sẵn thông qua lệnh ``pip search mxnet``.
...
`Thảo luận <https://discuss.mxnet.io/t/2315>`__
-----------------------------------------------
