...
python3 -m aws s3 sync --exclude '*.*' --include '*.css' \
     --content-type 'text/css' \
     --acl 'public-read'\
     $DIR $BUCKET

# --content-encoding 'gzip' \
python3 -m aws s3 sync --exclude '*.*' --include '*.js' \
     --content-type 'application/javascript' \
     --acl 'public-read'\
     $DIR $BUCKET

# use a large expire time for fonts
python3 -m aws s3 sync --exclude '*.*' --include '*.woff' --include '*.woff2' \
     --expires "2021-11-28T00:00:01Z" \
     --acl 'public-read'\
     $DIR $BUCKET

python3 -m aws s3 sync --delete $DIR $BUCKET --acl 'public-read'
